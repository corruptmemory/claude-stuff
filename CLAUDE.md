...
1. ssh-agent fix in `~/.local/bin/start-hyprland` (the launcher exports `SSH_AUTH_SOCK=$XDG_RUNTIME_DIR/ssh-agent.sock` and starts a dedicated agent there; see `~/projects/i3-screen-manager/docs/agent-guide/system.md`)
...
> **⚠️ 2026-09-22: on Brave Origin 153.1.95.104 the page API was ABSENT with both flags at `Default (Enabled)` — see the dated gotcha below. Unresolved until the flags are set explicitly and the browser relaunched.**
>
...
- **`--categoryExperimentalWebmcp` on Brave Origin 152.** This arg enables a dedicated WebMCP tool category inside chrome-devtools-mcp; it requires Chromium 149+ AND the `DevToolsWebMCPSupport` feature flag — both satisfied by Brave Origin 152 (`#devtools-webmcp-support` present and Enabled). Add it to the plugin args (alongside `--browserUrl` in step 4) to get the dedicated category. VERIFIED 2026-09-03 (chrome-devtools-mcp 1.8.0, Brave Origin 152): with step 4 applied, the plugin exposes a dedicated WebMCP category — `list_webmcp_tools` and `execute_webmcp_tool` — both now in the permissions block below. The 1.7.0 → 1.8.0 auto-update DID drop the patch (a fresh, unpatched version directory appeared beside the patched 1.7.0 one; `claude mcp list` showed a bare `npx chrome-devtools-mcp@1.8.0`); re-applied 2026-09-03. Dropped AGAIN by the 1.8.0 → 1.9.0 auto-update (fresh unpatched `1.9.0/` dir beside the patched `1.8.0/`; `claude mcp list` showed a bare `npx chrome-devtools-mcp@1.9.0`); re-applied 2026-09-14 with Brave Origin on Chromium 153 (flags still Enabled, `document.modelContext` probe still live). The "re-run step 4 after every update" rule is real, not theoretical — two for two so far, **and three for three on 2026-09-22, this time with NO version change:** the already-patched `1.9.0/` dir was bare again (`claude mcp list` showed a bare `npx chrome-devtools-mcp@1.9.0`; the session's tool list had no `list_webmcp_tools` / `execute_webmcp_tool`), so a marketplace refresh re-fetches the CURRENT version too, not only a new one. Re-applied 2026-09-22 — but the same day's Brave probe found the WebMCP page API itself GONE; see the next bullet. Raw CDP (`document.modelContext.getTools/executeTool`) remains proven independently (2026-08-28).
- **⚠️ 2026-09-22: the WebMCP PAGE API was ABSENT on Brave Origin 153.1.95.104 (Chromium 153.0.8010.53), with both flags at `Default (Enabled)`.** Measured, not inferred. `chrome://flags` lists `#enable-webmcp-testing` and `#devtools-webmcp-support` under Available, each reading `Default (Enabled)` — no longer explicitly set, and `Local State`'s `enabled_labs_experiments` is `[]` (on 09-14 both were explicit `@1`; the 'expect …@1' line in step 1 above therefore no longer holds on this build). The `brave` ELF still carries `enable-webmcp-testing`, `devtools-webmcp-support`, `kWebMCP`, `kWebMCPTesting`, `kDevToolsWebMCPSupport`, `WebMCPEvent`, `DeclarativeWebMCPTool`, so the feature was not removed. Yet on live https pages (mail.google.com, messages.google.com, youtube.com) an exhaustive scan of all 1380 window globals and every constructor prototype for `/model|mcp|agent|tool/i` found NO WebMCP surface: `document.modelContext`, `navigator.modelContext`, `window.ModelContext` and `WebMCPEvent` are all `undefined` (the only hits were `LanguageModel` — the Prompt API — `HTMLModElement`, `PannerNode.panningModel`, and Gmail's own `ToolbarApi`). Over CDP the `WebMCP` domain EXISTS — `WebMCP.enable` / `.disable` answer OK — but `isSupported`, `tools`, `invokeTool` come back `wasn't found`; chrome-devtools-mcp 1.9.0's `list_webmcp_tools` / `execute_webmcp_tool` go through Puppeteer's `page.webmcp.tools()`, which rides that domain, so they cannot work against this Brave even with the step-4 patch live. `WebMCP.enable` did not make the page API appear either. **Untested next step, and it needs a relaunch of Jim's browser, so it was left to him:** set both flags EXPLICITLY to `Enabled` (not Default) at `chrome://flags/#enable-webmcp-testing` and `chrome://flags/#devtools-webmcp-support`, relaunch, re-run the `document.modelContext` probe. Leading hypothesis: `Default (Enabled)` reports the base::Feature default while the Blink runtime feature that exposes the page API is only switched on by the explicit setting. If that fails, check whether upstream moved the API behind an origin trial in 153.
...
4. Check for `perplexity` in `claude mcp list`. If present, add the **perplexity block**. Scope gotcha (`godlike-artix`, 2026-09-18): perplexity is registered **project-local** — `claude mcp add`'s default scope, stored under `projects[<dir>].mcpServers` in `~/.claude.json`, ~30 dirs so far — so it only appears in `claude mcp list` when run from a directory it was added in. Absent from the list means "not added *here* yet", not "not installed"; `claude mcp add perplexity … --scope user` once would make it fleet-wide like open-brain. Re-added in `jai-http` 2026-09-18; block matched the live listing (4 tools) exactly. Re-added in `screen-killer` 2026-09-22 (14 dirs now); matched again.
...
8. Check for the claude.ai **Claude Docs** connector — `claude.ai Claude Docs` (`https://api.anthropic.com/v1/pages/mcp`) — in `claude mcp list`. If `✔ Connected`, add the **Claude Docs block**. Same claude.ai OAuth cloud-connector shape as the Google and Plaud ones (`mcp__claude_ai_Claude_Docs__<tool>`, not `claude mcp add`, not a plugin). Connected on `godlike-artix` 2026-09-18.
9. If any other MCP server shows up that prompts during use, enumerate its tools the same way and consider whether it's worth adding to this recipe for future projects.
   - 2026-09-14: `claude.ai Microsoft 365` and `claude.ai Slack` show up as `! Needs authentication` and expose only `authenticate` / `complete_authentication` (OAuth-flow stubs, outward-facing). Leave them OUT until one is actually connected and its real tool set is enumerated; then decide block-by-block like the Google ones.
...
**Drift check — do this every time you apply the recipe.** Plugins and connectors rename, add, and drop tools without notice (2026-09-04: playwright renamed `browser_run_code` → `browser_run_code_unsafe` and dropped `browser_install`; the Gmail connector grew `send_message`/`reply`/`forward`). The ground truth for current tool names is the session's own deferred-tool listing (the `mcp__…` names Claude Code prints at session start), NOT this file. Diff each block below against it before pasting. New read-only or reversible tools: add them here and in the project. Outward-facing (send mail, share files) or destructive (trash, delete) tools stay OUT so they keep prompting, whether they are new or already sit in a block, and say so in the summary — never silently auto-allow them. Applied to every block on 2026-09-18: `delete_thought`, Gmail `delete_label`, and Calendar `delete_event` were removed. One deliberate exception: `mcp__open-brain__coalesce_thoughts` stays auto-allowed. It deletes its inputs, but only by substituting one merged thought for them, and Jim judged Claude's use of it sound on average (2026-09-18). Blocks last re-verified against live tool names on `godlike-artix` **2026-09-18** — every existing block matched the live listing exactly (no renames, no drops); the only change that day was a new `claude.ai Claude Docs` connector, which got its own block below. A second pass later the same day, in `screen-killer` after `@playwright/mcp@latest` moved to 0.0.82, found real drift. Playwright gained `browser_emulate_media`, and its `browser_webmcp_*` pair went `skillOnly`, replaced by dynamic `webmcp_<name>` tools; details are in the Base block note. Gmail gained `delete_draft` (excluded). open-brain, chrome-devtools, Calendar, Drive, Plaud, and Claude Docs still matched exactly. **A third pass on 2026-09-22, again in `screen-killer`:** playwright and open-brain matched their blocks exactly, and `browser_wait_for` was BACK in the live listing (the 09-18 absence did not recur; it stays in the block). chrome-devtools was missing `list_webmcp_tools` / `execute_webmcp_tool` because the 1.9.0 manifest had lost the step-4 patch a THIRD time (details in the WebMCP gotchas); re-applied the same day, so they return next session. And NONE of the claude.ai connectors — Gmail, Drive, Calendar, Plaud, Claude Docs — appeared in `claude mcp list` or the session's tool listing: not `! Needs authentication`, simply absent, with `disableClaudeAiConnectors` set nowhere (no such key in `~/.claude/settings.json`, no `~/.claude/settings.local.json` exists, and `~/.claude.json` still carries `claudeAiMcpEverConnected`). Their blocks were left OUT per the 'add when ✔ Connected' rule. **Cause found later that day: the claude.ai login had lapsed.** After Jim ran `/login`, all five connectors (plus the `Microsoft 365` / `Slack` auth stubs) were back in the tool listing, every connector block matched it exactly (Gmail 30 live = 21 in the block + 9 excluded; Calendar 8 + 1; Drive 9 + 2; Plaud 5; Claude Docs 7 + 1), and the four blocks were merged into the project. **Rule learned: a connector that is simply absent — no `! Needs authentication` line, nothing in settings disabling it — means re-`/login`, not a config problem.** perplexity was absent too (registered in 13 project dirs, none of them `screen-killer`) until Jim added it there the same day, project-local as usual (14 dirs now); its block matched the live listing (4 tools) exactly.

---

### Base plugins block (always include)

Playwright tool set as of `@playwright/mcp@latest` on 2026-09-04: `browser_run_code` was renamed `browser_run_code_unsafe`, `browser_install` is gone (fine — this file already says never run playwright's installer on Arch), and `browser_drop`, `browser_find`, `browser_network_request` (singular, one request by id) were added. context7 is in the block on principle; it did not appear in `claude mcp list` on 2026-09-04, and unmatched entries are harmless. 2026-09-14 drift: playwright added `browser_webmcp_list` / `browser_webmcp_call` (WebMCP discovery + invoke through the Playwright plugin — the counterpart of chrome-devtools' `list_webmcp_tools` / `execute_webmcp_tool`); both added to the block, per the 🧟 WebMCP standing policy. **2026-09-18 drift (`@playwright/mcp` 0.0.82):** `browser_emulate_media` added (CSS media emulation, e.g. light/dark `colorScheme`; reversible, since `null` clears an override) and put in the block. `browser_webmcp_list` / `browser_webmcp_call` were REMOVED from the block. The source still defines them, but now as `skillOnly: true`, so they reach only the `playwright-cli` skill and never MCP. MCP now exposes each tool a page registers as its own dynamic MCP tool, `mcp__plugin_playwright_playwright__webmcp_<sanitized-name>` (announced by a tools-changed notification; `--no-webmcp` turns it off; WebMCP is on by default). Those names are page-specific, page-provided, and untrusted, with `destructiveHint` set unless the page marks the tool read-only, so they cannot be pre-enumerated and deliberately keep prompting. The auto-allowed WebMCP route is chrome-devtools' `list_webmcp_tools` / `execute_webmcp_tool`, which is unchanged. `browser_wait_for` was ABSENT from the live listing too, even though 0.0.82's source still has it as a plain `core`, non-`skillOnly` tool in `browserTools`. The cause wasn't found; it stays in the block (harmless if unmatched, right if it returns). **It returned: present in the live listing on 2026-09-22.**
...
Deliberately excluded (must keep prompting): `mcp__claude_ai_Gmail__send_message`, `…__reply`, `…__forward`, `…__trash_message`, `…__trash_thread`, `…__mark_message_spam`, `…__mark_thread_spam`, `…__delete_label`, `…__delete_draft`, `mcp__claude_ai_Google_Calendar__delete_event`, `mcp__claude_ai_Google_Drive__share_file`, `…__trash_file`. (`delete_draft` appeared 2026-09-18. Gmail's draft delete is permanent, not a trash, so it is excluded.)
...
### Claude Docs block (add when `claude.ai Claude Docs` is `✔ Connected` in `claude mcp list`)

Claude Docs is the claude.ai living-documents connector (`https://api.anthropic.com/v1/pages/mcp`) —
docs Claude creates and edits in place, private by default. Same claude.ai OAuth cloud-connector shape
as the Google and Plaud ones (`mcp__claude_ai_Claude_Docs__<tool>`, not `claude mcp add`, not a
plugin). First seen `✔ Connected` on `godlike-artix` 2026-09-18; tool names enumerated from the
session's deferred-tool listing the same day (eight tools). Seven are read-only (`guide`, `read`,
`query`, `export`) or edits to the user's own private docs (`batch`, `create`, `update`) — the same
class as Drive `create_file`/`update_file`, which the Google block already auto-allows — and are
included. **`delete` is destructive and is deliberately left OUT** so it keeps prompting, matching
the `trash_*` exclusions in the Google block.

Deliberately excluded (must keep prompting): `mcp__claude_ai_Claude_Docs__delete`.
...
- `glab repo create` makes projects **internal**, visible to every signed-in GitLab user, unless you pass `--private` or `--public`. Confirm before the first push: `glab api "projects/OWNER%2FNAME" | jq -r .visibility`
- In an existing repo, `glab repo create --skipGitInit` creates the project but adds no local remote; run `git remote add origin git@gitlab.com:OWNER/NAME.git` yourself
...
- **Align Markdown and Org tables in the source text.** Jim reads raw files in Emacs, not rendered pages, so an unaligned pipe table is a broken table. Pad every cell to its column's widest entry, extend the separator dashes to match, keep alignment colons, and count East Asian wide characters such as ✅ as two columns (Emacs `char-width` returns 2). A stdlib aligner with a `--check` mode, verified against Emacs `string-width`, lives at `~/lighthouse/connecting-accounts/tools/align-md-tables.py`. Added 2026-09-18.
- **Default git branch is `master`.** `init.defaultBranch` is `master`: keep the branch `git init` creates, and pass `--defaultBranch master` to `glab repo create`. The "Main branch: main" line in Claude Code's session context is a guess, not a preference. Repos already on `main`, such as `~/lighthouse/contract` and `~/lighthouse/connecting-accounts`, stay there; don't propose renaming them. Added 2026-09-18.
- Tests should be straightforward table-driven or sequential, using `t.TempDir()` for isolation
- **Package installs — AUR is hostile-by-default (post-2026 Atomic incident).** Six-rung decision procedure lives in `~/projects/i3-screen-manager/docs/agent-guide/packages.md` (Selection Policy; the longer original cheatsheet, `docs/install-paths-cheatsheet.md`, was removed 2026-09-06 and lives in that repo's git history): (1) Artix/Arch repos, (2) vendor's own native installer with auto-update, (3) `pipx` for Python tools, (4) `docker run` for occasional-use, (5) local PKGBUILD fork (the Odin trick, see Odin section below), (6) AUR direct after `aur-malware-check`. Worked examples: Claude Code / Codex / Brave Origin all took rung 2; Odin and `git-wd40` are rung 5 (`git-wd40` = Git with the WD-40 patchset, replaces the system `git` on both machines since 2026-09-13; fork at `~/projects/git-wd40`, sync procedure in its README — it keeps the AUR pkgname, so never let `yay` rebuild it from the AUR; install the built package with `sudo pacman -U --noconfirm --ask 4`). When suggesting an install path or when the user asks where to get something new, consult that guide rather than defaulting to `yay -S`. The trap classes (`sudo pip install`, `sudo npm install -g`, random Docker Hub images) look like a rung but aren't.
